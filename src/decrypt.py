...
def three_stage_cube_decrypt(img, Kr, Kc):
	Kr = np.array(Kr)
	Kc = np.array(Kc)
...
def two_stage_cube_decrypt(img, Kr, Kc):
	Kr = np.array(Kr)
	Kc = np.array(Kc)
...
def one_stage_cube_decrypt(img, Kr, Kc):
	Kr = np.array(Kr)
	Kc = np.array(Kc)
...
def decrypt(img, keyList):
    ...
    for key in keyList[::-1]:
        ...
        temp = three_stage_cube_decrypt(temp, Kr, Kc)
        temp = two_stage_cube_decrypt(temp, Kr, Kc)
        temp = one_stage_cube_decrypt(temp, Kr, Kc)
    return temp